...
int main(int argc, char* argv[]) {
    ...
    app.add_option("--nx", nx)
        ->required()
        ->description("X dimension(s)");
    
    app.add_option("--ny", ny)
        ->required()
        ->description("Y dimension(s)");
    
    app.add_option("--nz", nz)
        ->required()
        ->description("Z dimension(s)");
    ...
    if(filenames.size() != nx.size() || 
       filenames.size() != ny.size() ||
       filenames.size() != nz.size()) {
        std::cerr << "Number of images must match number of dimension parameters\n";
        return 1;
    }
